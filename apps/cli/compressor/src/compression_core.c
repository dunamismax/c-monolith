...
int compress_file(const char *input_path, const char *output_path, 
    ...
    switch (ctx->algorithm) {
        case ALGO_HUFFMAN:
            ...
            result = huffman_compress(input_data, input_size, 
        ...
        case ALGO_LZ77:
            ...
            result = lz77_compress(input_data, input_size, 
                                 &compressed_data, &compressed_size, ctx->level);
            break;
        ...
        case ALGO_HYBRID:
            ...
            size_t lz77_size = 0;
            result = lz77_compress(input_data, input_size, &lz77_data, &lz77_size, ctx->level);
            if (result == 0) {
                if (ctx->progress_callback) {
                    ctx->progress_callback(50.0, "Huffman post-processing");
                }
                result = huffman_compress(lz77_data, lz77_size, 
                                        &compressed_data, &compressed_size);
                free(lz77_data);
            }
            break;
    ...
    if (ctx->verbose) {
        print_compression_stats(input_size, compressed_size + sizeof(file_header_t), elapsed_time);
    }
...
int decompress_file(const char *input_path, const char *output_path,
    ...
    if (fread(compressed_data, 1, header.compressed_size, input_file) != header.compressed_size) {
    ...
    switch (header.algorithm) {
        ...
        case ALGO_HYBRID:
            ...
            if (result == 0) {
                ...
                result = lz77_decompress(huffman_data, huffman_size,
                                       &decompressed_data, &decompressed_size);
                free(huffman_data);
            }
            break;
    ...
    if (ctx->verbose) {
        ...
        switch (header.algorithm) {
            case ALGO_HUFFMAN: algo_name = "Huffman"; break;
            case ALGO_LZ77: algo_name = "LZ77"; break;
            case ALGO_HYBRID: algo_name = "Hybrid (LZ77+Huffman)"; break;
        }
