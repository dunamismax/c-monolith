...
lz77_match_t find_longest_match(const unsigned char *data, size_t pos,
                               size_t data_size, hash_entry_t **hash_table_param) {
    ...
    while (entry) {
        ...
        // Find match length
        uint16_t length = 0;
        size_t max_length = (data_size - pos > MAX_MATCH_LENGTH) ? 
                           MAX_MATCH_LENGTH : data_size - pos;
    ...
    match.next_char = (pos + best_length < data_size) ? 
                      data[pos + best_length] : 0;
...
int lz77_compress(const unsigned char *input, size_t input_size,
    ...
    while (pos < input_size) {
        ...
        if (match.length >= MIN_MATCH_LENGTH) {
            ...
            if (level > LEVEL_FAST) {
                for (uint16_t i = 1; i < match.length && pos - match.length + i + MIN_MATCH_LENGTH <= input_size; i++) {
...
int lz77_decompress(const unsigned char *input, size_t input_size,
    ...
    *output = malloc(output_capacity);
    ...
    while (bit_buf->byte_pos < input_size || bit_buf->bit_pos > 0) {
        ...
        if (bit_buf->byte_pos >= input_size) break;
