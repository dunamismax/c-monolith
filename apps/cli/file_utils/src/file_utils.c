...
static int validate_path(const char *path) {
    ...
    const char *dangerous_prefixes[] = {"/etc/",  "/usr/", "/bin/",  "/sbin/", "/boot/", "/sys/",
                                        "/proc/", "/dev/", "/root/", "/var/",  NULL};

    for (int i = 0; dangerous_prefixes[i]; i++) {
        if (strncmp(path, dangerous_prefixes[i], strlen(dangerous_prefixes[i])) == 0) {
            return -1;
        }
    }
...
static int validate_filename(const char *filename) {
    ...
    const char *reserved_names[] = {"CON",  "PRN",  "AUX",  "NUL",  "COM1", "COM2", "COM3", "COM4",
...
void list_directory(const char *dirname) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        int ret = snprintf(full_path, sizeof(full_path), "%s/%s", safe_path, entry->d_name);
