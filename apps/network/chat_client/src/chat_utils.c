...
int send_message_to_client(int client_socket, const chat_message_t* message) {
    ...
    ssize_t bytes_sent = send(client_socket, message, sizeof(chat_message_t), MSG_NOSIGNAL);
    if (bytes_sent < 0) {
        return -1;
    }
...
int receive_message_from_client(int client_socket, chat_message_t* message) {
    ...
    ssize_t bytes_received = recv(client_socket, message, sizeof(chat_message_t), 0);
    if (bytes_received <= 0) {
