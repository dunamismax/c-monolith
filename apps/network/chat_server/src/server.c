    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i].is_active && i != sender_index) {
            if (send_message_to_client(clients[i].socket_fd, message) < 0) {
                print_server_message("Failed to send message to client %s", clients[i].username);
            }
        }
    }
...
void broadcast_user_joined(const char* username) {
    ...
    snprintf(message.content, sizeof(message.content), "%s has joined the chat", username);
...
void broadcast_user_left(const char* username) {
    ...
    message.type = MSG_SERVER_INFO;
    message.timestamp = time(NULL);
    snprintf(message.content, sizeof(message.content), "%s has left the chat", username);
    strcpy(message.username, "SERVER");
...
void* handle_client(void* arg) {
    ...
    if (send_message_to_client(client->socket_fd, &welcome_msg) < 0) {
        print_server_message("Failed to send welcome message to %s", client->username);
...
int main(int argc, char* argv[]) {
    ...
    while (server_running) {
        ...
        int max_fd = (server_socket > shutdown_pipe[0]) ? server_socket : shutdown_pipe[0];
        ...
        if (client_index < 0) {
            print_server_message("Maximum clients reached. Connection rejected from %s", 
        ...
        if (receive_message_from_client(client_socket, &join_request) <= 0 || 
            ...
            print_server_message("Invalid join request from %s", inet_ntoa(client_addr.sin_addr));
            close(client_socket);
            continue;
        }
        
        if (strlen(join_request.username) == 0 || strlen(join_request.username) >= MAX_USERNAME_LENGTH) {
            print_server_message("Invalid username from %s", inet_ntoa(client_addr.sin_addr));
            
            chat_message_t error_msg = {0};
            error_msg.type = MSG_ERROR;
            error_msg.timestamp = time(NULL);
            strcpy(error_msg.username, "SERVER");
            strcpy(error_msg.content, "Invalid username. Please use 1-31 characters.");
        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            if (clients[i].is_active && strcmp(clients[i].username, join_request.username) == 0) {
                username_taken = true;
                break;
            }
        }
        ...
        if (username_taken) {
            print_server_message("Username '%s' already taken", join_request.username);
            
            chat_message_t error_msg = {0};
            error_msg.type = MSG_ERROR;
            error_msg.timestamp = time(NULL);
            strcpy(error_msg.username, "SERVER");
            strcpy(error_msg.content, "Username already taken. Please choose another.");
        ...
        *thread_arg = client_index;
        ...
        if (pthread_create(&clients[client_index].thread_id, NULL, handle_client, thread_arg) != 0) {
            print_server_message("Failed to create thread for client %s", join_request.username);
            cleanup_client(client_index);
            free(thread_arg);
            continue;
        }
