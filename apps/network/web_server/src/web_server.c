...
void send_error(int client_fd, int status_code, const char *status_text,
  ...
  send_response(client_fd, status_code, status_text, "text/html", body,
                body_len);
}
...
void parse_request_line(const char *request_line, char *method, char *path,
                        char *version) {
...
void *handle_client(void *arg) {
  ...
  inet_ntop(AF_INET, &(client_info->client_addr.sin_addr), client_ip,
            INET_ADDRSTRLEN);
