...
typedef enum { VIEW_PROCESSES, VIEW_TREE, VIEW_SYSTEM } view_mode_t;
...
bool read_proc_stat(const char *pid_str, process_info_t *proc) {
  ...
  int ret = fscanf(file,
                   ...
                   "%*d %*d %ld %ld %*d %*d %lu %lu %ld",
                   &proc->pid, comm, &proc->state, &proc->ppid, &proc->utime,
                   &proc->stime, &proc->priority, &proc->nice, &proc->starttime,
                   &proc->vsize, &proc->rss);
...
bool read_system_loadavg(system_info_t *sys) {
  ...
  int ret = fscanf(file, "%lf %lf %lf", &sys->load_avg[0], &sys->load_avg[1],
                   &sys->load_avg[2]);
  fclose(file);
...
void calculate_memory_usage(process_info_t *proc, const system_info_t *sys) {
  if (sys->total_memory > 0) {
    proc->memory_percent =
        (double)(proc->rss * 4) / (sys->total_memory) * 100.0;
  } else {
...
bool update_processes(monitor_state_t *state) {
  ...
  if (stat_file) {
    ...
    if (fgets(line, sizeof(line), stat_file)) {
      ...
      if (sscanf(line, "cpu %lu %lu %lu %lu %lu %lu %lu %lu", &user, &nice,
    ...
    for (int j = 0; j < state->count; j++) {
      if (state->processes[j].ppid == state->processes[i].pid) {
        if (state->processes[i].children_count < 64) {
          state->processes[i].children[state->processes[i].children_count] =
              state->processes[j].pid;
...
bool update_system_info(system_info_t *sys) {
  if (!read_system_meminfo(sys))
    return false;
  if (!read_system_loadavg(sys))
    return false;
  if (!read_system_uptime(sys))
    return false;
...
int compare_processes(const void *a, const void *b) {
  ...
  case SORT_CPU:
    if (proc_b->cpu_percent > proc_a->cpu_percent)
      return 1;
    if (proc_b->cpu_percent < proc_a->cpu_percent)
      return -1;
    return 0;
  case SORT_MEMORY:
    if (proc_b->memory_percent > proc_a->memory_percent)
      return 1;
    if (proc_b->memory_percent < proc_a->memory_percent)
      return -1;
    return 0;
...
void sort_processes(monitor_state_t *state) {
  qsort(state->processes, state->count, sizeof(process_info_t),
        compare_processes);
}
...
bool matches_filter(const process_info_t *proc, const char *filter) {
  ...
  return strstr(proc->command, filter) != NULL ||
         strstr(pid_str, filter) != NULL;
}
...
void draw_system_info(int start_row) {
  ...
  mvprintw(start_row + 2, 2, "Load Average: %.2f %.2f %.2f",
           state.system.load_avg[0], state.system.load_avg[1],
           state.system.load_avg[2]);
  mvprintw(start_row + 3, 2, "CPU Usage: %.1f%%", state.system.cpu_usage);
  mvprintw(start_row + 4, 2,
           "Memory: %lu MB total, %lu MB free, %lu MB available",
...
void draw_process_line(int row, int index __attribute__((unused)),
                       const process_info_t *proc, bool selected) {
  if (selected) {
    attron(A_REVERSE);
  }

  mvprintw(row, 0, "%5d %-8s %3ld %3ld %7lu %6ld %6s %c %5.1f %4.1f %9s %s",
           proc->pid, "user", proc->priority, proc->nice, proc->vsize / 1024,
           proc->rss * 4, "0", proc->state, proc->cpu_percent,
           proc->memory_percent, "0:00.00", proc->command);
...
void draw_tree_process(int row, int depth, const process_info_t *proc,
                       bool selected) {
  if (selected) {
    attron(A_REVERSE);
  }
  ...
  mvprintw(row, 0, "%s%5d %5.1f%% %4.1f%% %s", indent, proc->pid,
           proc->cpu_percent, proc->memory_percent, proc->command);
...
void draw_tree_recursive(int *current_row, int max_row, pid_t ppid, int depth) {
  ...
  for (int i = 0; i < state.count; i++) {
    if (state.processes[i].ppid == ppid &&
        matches_filter(&state.processes[i], state.filter)) {
      ...
      draw_tree_recursive(current_row, max_row, state.processes[i].pid,
                          depth + 1);
    }
  }
}
  ...
  if (state.view_mode == VIEW_TREE) {
    ...
    for (int i = 0; i < state.count && displayed < max_rows; i++) {
      if (matches_filter(&state.processes[i], state.filter)) {
        bool selected = (i == state.selected_process);
        draw_process_line(start_row + displayed, i, &state.processes[i],
                          selected);
        displayed++;
      }
    }
...
void draw_status_line(void) {
  ...
  mvprintw(max_y - 1, 0, "Sort: %s | View: %s | Filter: '%s' | Processes: %d",
           ...
           : (state.sort_mode == SORT_TIME)   ? "Time"
                                              : "Command",
           (state.view_mode == VIEW_TREE) ? "Tree" : "List", state.filter,
           state.count);
...
void handle_input(void) {
  ...
  case 'T':
    state.view_mode =
        (state.view_mode == VIEW_TREE) ? VIEW_PROCESSES : VIEW_TREE;
    break;
...
void cleanup_monitor(void) { endwin(); }
