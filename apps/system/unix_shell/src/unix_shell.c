...
char **expand_wildcards(char **args, int *argc) {
  ...
  for (int i = 0; i < *argc; i++) {
    if (strchr(args[i], '*') || strchr(args[i], '?')) {
      ...
      if (ret == 0) {
        for (size_t j = 0; j < glob_result.gl_pathc && new_argc < MAX_ARGS - 1;
             j++) {
...
builtin_t builtins[] = {{"cd", builtin_cd},       {"pwd", builtin_pwd},
                        {"exit", builtin_exit},   {"help", builtin_help},
                        {"jobs", builtin_jobs},   {"history", builtin_history},
                        {"env", builtin_env},     {"export", builtin_export},
...
char *command_generator(const char *text, int state) {
  static char *commands[] = {"cd",      "pwd",  "exit",   "help",  "jobs",
                             "history", "env",  "export", "unset", "ls",
                             "cat",     "grep", "find",   "ps",    "top",
                             "kill",    "echo", "which",  "man",   NULL};
...
void init_shell(void) {
  ...
  if (home) {
    ...
    snprintf(history_file, sizeof(history_file), "%s/.unix_shell_history",
             home);
...
void print_prompt(void) {
  ...
  printf("\033[32m%s@%s\033[0m:\033[34m%s\033[0m$ ", user, hostname,
         display_path);
  free(display_path);
  fflush(stdout);
}
