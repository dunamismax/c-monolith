...
bool delete_at_position(LinkedList* list, int position) {
    if (!list || list->head == NULL || position < 0) return false;
...
int array_get(DynamicArray* arr, int index) {
    if (!arr || index < 0 || index >= arr->size) return -1;
...
void bfs(Graph* graph, int start_vertex) {
    if (!graph || start_vertex < 0 || start_vertex >= graph->num_vertices) return;
...
int string_to_integer(const char* str) {
    if (!str || !is_valid_integer(str)) return 0;
