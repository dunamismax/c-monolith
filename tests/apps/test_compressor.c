...
// Test data
static const char *test_string_small =
    "Hello, World! This is a test string for compression.";
static const char *test_string_repetitive =
    "aaaaaabbbbbbccccccddddddeeeeeeffffffgggggghhhhhhiiiiiijjjjjjkkkkkk";
...
int test_crc32_calculation(void) {
  ...
  uint32_t crc =
      calculate_crc32((const unsigned char *)test_data, strlen(test_data));
...
int test_huffman_compression(void) {
  ...
  // Test compression
  int result =
      huffman_compress(input, input_size, &compressed, &compressed_size);
  if (result != 0)
    return 0;
  if (!compressed)
    return 0;
  ...
  result = huffman_decompress(compressed, compressed_size, &decompressed,
                              &decompressed_size);
  if (result != 0) {
    free(compressed);
    return 0;
...
int test_file_compression_decompression(void) {
  ...
  char *decompressed_content =
      read_file_contents(decompressed_path, &decompressed_size);
  ...
  if (!original_content || !decompressed_content) {
    ...
    success = 0;
  } else if (memcmp(original_content, decompressed_content, original_size) !=
             0) {
    success = 0;
  }
...
int test_error_handling(void) {
  ...
  int result =
      compress_file("/tmp/nonexistent_file_12345", "/tmp/output.comp", &ctx);
  if (result == 0)
...
int main(void) {
  ...
  run_test(test_file_compression_decompression,
