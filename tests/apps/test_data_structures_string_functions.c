...
int test_string_length(void) {
  ...
  if (string_length("a") != 1)
    return 0;
  if (string_length("Hello, World!") != 13)
    return 0;
  if (string_length(NULL) != 0)
    return 0;
  return 1;
}
...
int test_string_compare(void) {
  // Equal strings
  if (string_compare("hello", "hello") != 0)
    return 0;
  if (string_compare("", "") != 0)
    return 0;

  // First string less than second
  if (string_compare("apple", "banana") >= 0)
    return 0;
  if (string_compare("a", "b") >= 0)
    return 0;

  // First string greater than second
  if (string_compare("banana", "apple") <= 0)
    return 0;
  if (string_compare("b", "a") <= 0)
    return 0;

  // Different lengths
  if (string_compare("cat", "cats") >= 0)
    return 0;
  if (string_compare("cats", "cat") <= 0)
    return 0;

  // Test with NULL (should handle gracefully)
  if (string_compare(NULL, "test") != 0)
    return 0;
  if (string_compare("test", NULL) != 0)
    return 0;
  if (string_compare(NULL, NULL) != 0)
    return 0;
...
int test_string_to_integer(void) {
  // Test positive numbers
  if (string_to_integer("123") != 123)
    return 0;
  if (string_to_integer("0") != 0)
    return 0;
  if (string_to_integer("1") != 1)
    return 0;

  // Test negative numbers
  if (string_to_integer("-456") != -456)
    return 0;
  if (string_to_integer("-1") != -1)
    return 0;

  // Test with plus sign
  if (string_to_integer("+789") != 789)
    return 0;

  // Test invalid strings (should return 0)
  if (string_to_integer("abc") != 0)
    return 0;
  if (string_to_integer("12abc") != 0)
    return 0;
  if (string_to_integer("") != 0)
    return 0;
  if (string_to_integer(NULL) != 0)
    return 0;
...
int test_is_valid_integer(void) {
  // Valid integers
  if (!is_valid_integer("123"))
    return 0;
  if (!is_valid_integer("0"))
    return 0;
  if (!is_valid_integer("-456"))
    return 0;
  if (!is_valid_integer("+789"))
    return 0;
  if (!is_valid_integer("1"))
    return 0;

  // Invalid integers
  if (is_valid_integer("abc"))
    return 0;
  if (is_valid_integer("12abc"))
    return 0;
  if (is_valid_integer(""))
    return 0;
  if (is_valid_integer("-"))
...
int test_special_characters(void) {
  ...
  if (string_length("Hello\nWorld") != 11)
    return 0;
  if (string_length("Tab\tCharacter") != 13)
    return 0; // "Tab" + "\t" + "Character" = 3 + 1 + 9 = 13
...
int test_long_strings(void) {
  ...
  if (string_length(long_str) != 999)
    return 0;

  string_copy(long_str, dest);
  if (strcmp(dest, long_str) != 0)
    return 0;
...
int test_edge_cases(void) {
  ...
  if (string_compare("a", "aa") >= 0)
    return 0;
  if (string_compare("aa", "a") <= 0)
    return 0;
